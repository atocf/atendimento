...
public class ChamadoExcelExporter {
	...
	public void export(HttpServletResponse response, String nameSheet, String[] namesCell) throws IOException, ParseException {
	...
	private void writeDataLines(String tipo) throws ParseException {
		...
		for (Chamado chamado : list) {
			if(tipo.equals("Atendimento PF")) {
				...
				createCell(row, columnCount++, DataUtils.format(chamado.getDataabertura(), DataUtils.formatoData), style);
				createCell(row, columnCount++, DataUtils.format(chamado.getDatavencimento(), DataUtils.formatoData), style);
				...
				createCell(row, columnCount++, DataUtils.format(chamado.getDataatualizacaocadastral(), DataUtils.formatoData), style);
			...
			} else if(tipo.equals("Sincronizar") && chamado.getConta().size() > 0) {
