...
import br.com.atendimento.repository.MsgRepository;
...
public class MsgService {
	...
	public Page<Msg> findAll(int page, int size, Boolean ativo) {
		log.info("Buscando a mensagem ordenando pelo nome");
	...
	public Optional<Msg> findById(Long id) {
		log.info("Buscando a mensagem por id: {}", id);
		return repository.findById(id);
	}
	
	public Msg save(Msg x) {
		log.info("Salvando ou Atualização a mensagem {}", x.getNome());
		return repository.save(x);
	}

	public void deleteById(Long id) {
		log.info("Deletando a mensagem por id: {}", id);
		repository.deleteById(id);
	}
	
	public ErrorObject returnError(Long id) {
		return new ErrorObject("Mensagem selecionado não existe", "status_id", id);
	}
}