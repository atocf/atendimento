...
public class SquadService {
	...
	public Page<Squad> findAll(int page, int size, Boolean ativo) {
		log.info("Buscando a squad ordenando pelo nome");
	...
	public Optional<Squad> findById(Long id) {
		log.info("Buscando a squad por id: {}", id);
		return repository.findById(id);
	}

	public Optional<Squad> findByNome(String nome) {
		log.info("Buscando a squad pelo nome: {}", nome);
		return repository.findByNome(nome);
	}
	
	public Squad save(Squad x) {
		log.info("Salvando ou Atualização a squad {}", x.getNome());
		return repository.save(x);
	}

	public void deleteById(Long id) {
		log.info("Deletando a squad por id: {}", id);
